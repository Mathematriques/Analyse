...
\proposition
La topologie codén\up{ble} sur $E$ est $\mathscr{T}=\{\vide\}\cup\{X\subset E|\complement X\text{ dénombrable}\}$.\demonstration
\begin{itemise}
...
\iffalse
\impldir Soit $O$ un ouvert non vide et $x\in O$.
Alors $O$ est un voisinage de $x$, et $x\in\overline{X}$ donc $O\cap X\neq\vide$.

\implrec Soit $O$ un ouvert de $E$ inclus dans $\complement X$.
