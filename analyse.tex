...
\begin{demonstration}
	L'application $\complement : X\longmapsto E\setminus X$ est une bijection car c'est une involution.
	Soit $\mathscr{O}$ une topologie d'ouverts et montrons que $\mathscr{F}=\complement\mathscr{O}$ est une topologie de fermés.
	\begin{itemise}
		\item si $(F_i)_{1\infegal i\infegal n}$ une famille finie de $\mathscr{F}$ alors
		$\bigcup_{i=1}^nF_i=\bigcup_{i=1}^n\complement O_i=\complement\bigcap_{i=1}^nO_i\in\mathscr{F}$.
		\item si $(F_i)_{i\in I}$ une famille quelconque de $\mathscr{F}$ alors
...
\begin{definition}
	...
	\begin{itemise}
		\item L'ensemble $\mathfrak{V}(x)$ contient l'espace complet.
		\item Les ensembles de $\mathfrak{V}(x)$ contiennent chacun $x$.
		\item Toute intersection finie d'ensemble de $\mathfrak{V}(x)$ est un ensemble de $\mathfrak{V}(x)$.
		\item $\forall V\in\mathfrak{V}(x)\quad\left[V\subset W\subset E~\Rightarrow~W\in\mathfrak{V}(x)\right]$
...
\begin{demonstration}
	Soit $\Phi:\mathscr{O}\mapsto(x\mapsto\ens{V\subset E}{\exists O\in\mathscr{O},\ x\in O\subset V})$ allant de l'ensemble des topologies d'ouverts vers l'ensemble des voisinaginations
	Nous allons montrer qu'elle estbijective en montrant l'injectivité puis la surjectivité.
	\begin{itemise}
		\item Montrons que $\Phi$ est injective.
		Soient deux topologies $\mathscr{O}_1$ et $\mathscr{O}_2$ tels que $\Phi(\mathscr{O}_1)=\Phi(\mathscr{O}_2)$.
		...
		Ainsi il existe un ouvert $O_{2, x}\in\mathscr{O}_2$ tel que $x\in O_2^x\subset O_1$.
		...
		\item Montrons que $\Phi$ est surjective en montrant qu'un antécédent par $\Phi$ de $\mathfrak{V}$ un voisinagination est $\mathscr{O}=\ens{O\subset E}{\forall x\in O, O\in\mathfrak{V}(x)}$.
		\begin{itemise}
			\item Vérifions donc que $\mathscr{O}$ est une topologie d'ouverts.
			\begin{itemise}
				\item Soit $(O_i)_{i\in I}$ une famille de $\mathscr{O}$ et $O=\in\bigcup_{i\in I}O_i$.
