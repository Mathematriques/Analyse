...
% 	\iff
% 	\scr{T}=\sigma_\text{topologie}(\frak{B})
% 	$$
% \end{remarque}
...
\begin{figure}[h]
	...
	\begin{tikzpicture}[scale=6]
		...
		\draw[thick] (\xf, \yf) -- (\xf+\hf, \yf+\hf) -- (\xf+\hf+\wf, \yf+\hf-\wf) -- (\xf+\wf, \yf-\wf) node[anchor=south, minimum height=1cm] {$F$} -- cycle;
		% G
		\pgfmathsetmacro{\xg}{0.6}
		\pgfmathsetmacro{\yg}{0.55}
		\pgfmathsetmacro{\hg}{0.15}
		\pgfmathsetmacro{\wg}{0.2}
		\draw[thick] (\xg, \yg) -- (\xg+\hg, \yg+\hg) node[anchor=north, minimum height=0.75cm] {$G$} -- (\xg+\hg+\wg, \yg+\hg) -- (\xg+\wg, \yg) -- cycle;
		% point de F°G
...
\begin{definition}
	À partir d'une structure quasi-uniforme $\mathscr{U}$ on définit une topologie dite topologie engendrée par $\mathscr{U}$ en appelant voisinage d'un point $x$ les ensembles de la forme $U[x]$ avec $U\in\mathscr{U}$.
...
\begin{proposition}
	...
	La famille suivante est une prébase d'uniformité appelée la prébase de Pervin (1962) $\frak{A}_\text{Pervin}=\ens{R_U=(U\times U)\cup(\complement U\times E)}{U\in\scr{T}}$.
	Cette prébase engendre la topologie d'origine $\scr{T}$.
\end{proposition}

La relation $R_U$ dit simplement $x\in U\implies y\in U$.
Cela permet de montrer facilement la transitivité de $R_U$ c'est-à-dire $R_U\circ R_U\subset R_U$.
En effet si $(x, z)$ appartient à $R_U\circ R_U$ il existe un $y$ tel que $(x, y), (y, z)\in R_U$ donc $x\in U\implies y\in U$ et $y\in U\implies z\in U$.
Par transitivité de l'implication $x\in U\implies z\in U$ donc $(x, z)\in R_U$.
...
\begin{proposition}
	Un espace topologie $E$ est $T_3$ si et seulement si sa quasi-uniformité de Pervin vérifie $\forall U\in\scr{U}, \forall x\in E, \exists V\in\scr{U}, V=\inv{V}\et (V\circ V)[x]\subset U[x]$
\end{proposition}

\equivalence{Par double implication.}{
	Soit $U\in\scr{U}$ et $x\in E$.
	$U[x]$ est un voisinage de $x$ donc $\exists O\in\scr{T}, x\in O\subset U[x]$.
	Or $x\notin\complement O$ fermé donc par $T_3$ $\exists U, \complement F\in\scr{T}, x\in U\et\complement O\subset\complement F\et U\cap\complement F=\vide$ ce qui s'écrit aussi $x\in U\subset F\subset O$.
...
\begin{demonstration}
	Posons $\Phi:\scr{T}\mapsto\sigma_\text{quasi}(\frak{B})$ avec $\frak{B}$ la famille des intersections finies de $\frak{B}'=\ens{O^2\cup\complement O\times E}{O\in\scr{T}}$ (appelée prébase de Pervin).
...
\begin{proposition}
	Si $\frak{B}$ est une base $\sigma_\text{topologie}(\frak{B})=\{U\subset E| \exists X_{i\in I}\in\frak{B}, U=\bigcup_{i\in I}X_i}$
\end{proposition}

\begin{demonstration}
	Notons l'ensemble de droite $\scr{T}=\ens{U\subset E}{\exists X_{i\in I}\in\frak{B}, U=\bigcup_{i\in I}X_i}$ et montrons $\scr{T}=\sigma_\text{topologie}(\frak{B})$ par double inclusion.
	\begin{itemise}
		\item[$\subset$] Soit $O\in\scr{T}$ alors $\exists X_{i\in I}\in\frak{B}, O=\bigcup_{i\in I}X_i$.
		Or $\sigma_\text{topologie}(\frak{B})$ continent $\frak{B}$ donc $X_{i\in I}\in\sigma_\text{topologie}(\frak{B})$, et est une topologie donc $O=\bigcup_{i\in I}X_i\in\sigma_\text{topologie}(\frak{B})$.
		...
		Or $\sigma_\text{topologie}(\frak{B})$ est la plus petite topologie contenant $\frak{B}$.
		Donc $\scr{T}\supset\sigma_\text{topologie}(\frak{B})$.
	\end{itemise}
\end{demonstration}

Soit $\frak{V}$ une voisinagination de $E$.
Vérifions que $\scr{T}_\text{ouvert}=\Phi(\frak{V})$ est une topologie d'ouverts.
\begin{itemise}
	\item $\forall x\in\vide, \vide\in\frak{V}(x)$ est vrai donc $\vide\in\scr{T}_\text{ouvert}$ et $E\in\scr{T}_\text{ouvert}$ par hypothèse.
	\item Si $O, O'\in\scr{T}_\text{ouvert}$ alors $x\in O\cap O'$ implique $x\in O\et x\in O'$ donc $O\in\frak{V}(x)\et O'\in\frak{V}(x)$, soit $O, O'\in\frak{V}(x)$.
	Par définition des voisinaginations $O\cap O'\in\frak{V}(x)$ soit $O\cap O'\in\scr{T}_\text{ouvert}$.
	\item Si $O_{i\in I}\in\scr{T}_\text{ouvert}$ alors $x\in\bigcup_{i\in I} O_i$ implique $\exists i\in I, x\in O_i$.
	Ainsi $O_i\in\frak{V}(x)\et O_i\subset\bigcup_{i\in I} O_i$, donc par définition des voisinaginations $\bigcup_{i\in I} O_i\in\frak{V}(x)$ soit $\bigcup_{i\in I} O_i\in\scr{T}_\text{ouvert}$.
\end{itemise}

$$
	\scr{T}_\text{ouvert}\underset{\Psi}{\longmapsto}\frak{V}\underset{\Phi}{\longmapsto}\scr{T}_\text{ouvert}'
$$

Vérifions que $\Psi\circ\Phi=\id$.
Soit $\scr{T}_\text{ouvert}$ une topologie d'ouverts de $E$.
Vérifions que $\scr{T}_\text{ouvert}'=\scr{T}_\text{ouvert}$.

$$
	\frak{V}\underset{\Phi}{\longmapsto}\scr{T}_\text{ouvert}\underset{\Psi}{\longmapsto}\frak{V}'
$$
Soit $\frak{V}$ une voisinagination de $E$.
...
\begin{itemise}
	...
	Si $y\in O$ alors $V\in\frak{V}(y)$ donc $y\in V$ soit $O\subset V$, et $\exists W\in\frak{V}(y), \forall z\in W, V\in\frak{V}(z)$ soit $z\in O$  donc $W\subset O$ puis $O\in\frak{V}(y)$ par définition de voisinagination, donc $O\in\Psi(\frak{V})=\scr{T}_\text{ouvert}$.
	Ainsi $\exists O\in\scr{T}_\text{ouvert}, x\in O\subset V$ donc $V\in\Psi(\scr{T}_\text{ouvert})(x)=\frak{V}'(x)$.
	\item Si $V'\in\frak{V}'(x)$ alors $\exists O\in\scr{T}_\text{ouvert}, x\in O\subset V'$.
	Or $\scr{T}_\text{ouvert}=\Phi(\frak{V})$ donc $O\in\frak{V}(x)$, puis $V'\in\frak{V}(x)$ par définition d'une voisinagination.
\end{itemise}
